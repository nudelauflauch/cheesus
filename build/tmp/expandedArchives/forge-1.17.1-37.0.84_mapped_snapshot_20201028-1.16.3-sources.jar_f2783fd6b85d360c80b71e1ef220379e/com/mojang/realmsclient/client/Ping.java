package com.mojang.realmsclient.client;

import com.google.common.collect.Lists;
import com.mojang.realmsclient.dto.RegionPingResult;
import java.net.InetSocketAddress;
import java.net.Socket;
import java.net.SocketAddress;
import java.util.Comparator;
import java.util.List;
import net.minecraft.Util;
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.api.distmarker.OnlyIn;

@OnlyIn(Dist.CLIENT)
public class Ping {
   public static List<RegionPingResult> m_87130_(Ping.Region... p_87131_) {
      for(Ping.Region ping$region : p_87131_) {
         m_87126_(ping$region.f_87142_);
      }

      List<RegionPingResult> list = Lists.newArrayList();

      for(Ping.Region ping$region1 : p_87131_) {
         list.add(new RegionPingResult(ping$region1.f_87141_, m_87126_(ping$region1.f_87142_)));
      }

      list.sort(Comparator.comparingInt(RegionPingResult::m_87652_));
      return list;
   }

   private static int m_87126_(String p_87127_) {
      int i = 700;
      long j = 0L;
      Socket socket = null;

      for(int k = 0; k < 5; ++k) {
         try {
            SocketAddress socketaddress = new InetSocketAddress(p_87127_, 80);
            socket = new Socket();
            long l = m_87132_();
            socket.connect(socketaddress, 700);
            j += m_87132_() - l;
         } catch (Exception exception) {
            j += 700L;
         } finally {
            m_87128_(socket);
         }
      }

      return (int)((double)j / 5.0D);
   }

   private static void m_87128_(Socket p_87129_) {
      try {
         if (p_87129_ != null) {
            p_87129_.close();
         }
      } catch (Throwable throwable) {
      }

   }

   private static long m_87132_() {
      return Util.m_137550_();
   }

   public static List<RegionPingResult> m_87125_() {
      return m_87130_(Ping.Region.values());
   }

   @OnlyIn(Dist.CLIENT)
   static enum Region {
      US_EAST_1("us-east-1", "ec2.us-east-1.amazonaws.com"),
      US_WEST_2("us-west-2", "ec2.us-west-2.amazonaws.com"),
      US_WEST_1("us-west-1", "ec2.us-west-1.amazonaws.com"),
      EU_WEST_1("eu-west-1", "ec2.eu-west-1.amazonaws.com"),
      AP_SOUTHEAST_1("ap-southeast-1", "ec2.ap-southeast-1.amazonaws.com"),
      AP_SOUTHEAST_2("ap-southeast-2", "ec2.ap-southeast-2.amazonaws.com"),
      AP_NORTHEAST_1("ap-northeast-1", "ec2.ap-northeast-1.amazonaws.com"),
      SA_EAST_1("sa-east-1", "ec2.sa-east-1.amazonaws.com");

      final String f_87141_;
      final String f_87142_;

      private Region(String p_87148_, String p_87149_) {
         this.f_87141_ = p_87148_;
         this.f_87142_ = p_87149_;
      }
   }
}