...
public class SkinProcessor {
   ...
   @Nullable
   public BufferedImage m_90241_(BufferedImage p_90242_) {
      if (p_90242_ == null) {
         return null;
      } else {
         this.f_90233_ = 64;
         this.f_90234_ = 64;
         BufferedImage bufferedimage = new BufferedImage(this.f_90233_, this.f_90234_, 2);
         ...
         graphics.dispose();
         this.f_90232_ = ((DataBufferInt)bufferedimage.getRaster().getDataBuffer()).getData();
   ...
   private void m_90236_(int p_90237_, int p_90238_, int p_90239_, int p_90240_) {
      for(int i = p_90237_; i < p_90239_; ++i) {
         for(int j = p_90238_; j < p_90240_; ++j) {
            int k = this.f_90232_[i + j * this.f_90233_];
            if ((k >> 24 & 255) < 128) {
               return;
            }
         }
      ...
      for(int l = p_90237_; l < p_90239_; ++l) {
         for(int i1 = p_90238_; i1 < p_90240_; ++i1) {
            this.f_90232_[l + i1 * this.f_90233_] &= 16777215;
         }
      }
   ...
   private void m_90243_(int p_90244_, int p_90245_, int p_90246_, int p_90247_) {
      for(int i = p_90244_; i < p_90246_; ++i) {
         for(int j = p_90245_; j < p_90247_; ++j) {
            this.f_90232_[i + j * this.f_90233_] |= -16777216;
         }
      }
