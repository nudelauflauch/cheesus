package com.mojang.realmsclient.util;

import com.google.common.annotations.VisibleForTesting;
import com.google.common.collect.Lists;
import java.util.Arrays;
import java.util.List;
import java.util.Objects;
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.api.distmarker.OnlyIn;

@OnlyIn(Dist.CLIENT)
public class TextRenderingUtils {
   private TextRenderingUtils() {
   }

   @VisibleForTesting
   protected static List<String> m_90248_(String p_90249_) {
      return Arrays.asList(p_90249_.split("\\n"));
   }

   public static List<TextRenderingUtils.Line> m_90256_(String p_90257_, TextRenderingUtils.LineSegment... p_90258_) {
      return m_90253_(p_90257_, Arrays.asList(p_90258_));
   }

   private static List<TextRenderingUtils.Line> m_90253_(String p_90254_, List<TextRenderingUtils.LineSegment> p_90255_) {
      List<String> list = m_90248_(p_90254_);
      return m_90259_(list, p_90255_);
   }

   private static List<TextRenderingUtils.Line> m_90259_(List<String> p_90260_, List<TextRenderingUtils.LineSegment> p_90261_) {
      int i = 0;
      List<TextRenderingUtils.Line> list = Lists.newArrayList();

      for(String s : p_90260_) {
         List<TextRenderingUtils.LineSegment> list1 = Lists.newArrayList();

         for(String s1 : m_90250_(s, "%link")) {
            if ("%link".equals(s1)) {
               list1.add(p_90261_.get(i++));
            } else {
               list1.add(TextRenderingUtils.LineSegment.m_90279_(s1));
            }
         }

         list.add(new TextRenderingUtils.Line(list1));
      }

      return list;
   }

   public static List<String> m_90250_(String p_90251_, String p_90252_) {
      if (p_90252_.isEmpty()) {
         throw new IllegalArgumentException("Delimiter cannot be the empty string");
      } else {
         List<String> list = Lists.newArrayList();

         int i;
         int j;
         for(i = 0; (j = p_90251_.indexOf(p_90252_, i)) != -1; i = j + p_90252_.length()) {
            if (j > i) {
               list.add(p_90251_.substring(i, j));
            }

            list.add(p_90252_);
         }

         if (i < p_90251_.length()) {
            list.add(p_90251_.substring(i));
         }

         return list;
      }
   }

   @OnlyIn(Dist.CLIENT)
   public static class Line {
      public final List<TextRenderingUtils.LineSegment> f_90262_;

      Line(TextRenderingUtils.LineSegment... p_167625_) {
         this(Arrays.asList(p_167625_));
      }

      Line(List<TextRenderingUtils.LineSegment> p_90264_) {
         this.f_90262_ = p_90264_;
      }

      public String toString() {
         return "Line{segments=" + this.f_90262_ + "}";
      }

      public boolean equals(Object p_90266_) {
         if (this == p_90266_) {
            return true;
         } else if (p_90266_ != null && this.getClass() == p_90266_.getClass()) {
            TextRenderingUtils.Line textrenderingutils$line = (TextRenderingUtils.Line)p_90266_;
            return Objects.equals(this.f_90262_, textrenderingutils$line.f_90262_);
         } else {
            return false;
         }
      }

      public int hashCode() {
         return Objects.hash(this.f_90262_);
      }
   }

   @OnlyIn(Dist.CLIENT)
   public static class LineSegment {
      private final String f_90269_;
      private final String f_90270_;
      private final String f_90271_;

      private LineSegment(String p_90273_) {
         this.f_90269_ = p_90273_;
         this.f_90270_ = null;
         this.f_90271_ = null;
      }

      private LineSegment(String p_90275_, String p_90276_, String p_90277_) {
         this.f_90269_ = p_90275_;
         this.f_90270_ = p_90276_;
         this.f_90271_ = p_90277_;
      }

      public boolean equals(Object p_90287_) {
         if (this == p_90287_) {
            return true;
         } else if (p_90287_ != null && this.getClass() == p_90287_.getClass()) {
            TextRenderingUtils.LineSegment textrenderingutils$linesegment = (TextRenderingUtils.LineSegment)p_90287_;
            return Objects.equals(this.f_90269_, textrenderingutils$linesegment.f_90269_) && Objects.equals(this.f_90270_, textrenderingutils$linesegment.f_90270_) && Objects.equals(this.f_90271_, textrenderingutils$linesegment.f_90271_);
         } else {
            return false;
         }
      }

      public int hashCode() {
         return Objects.hash(this.f_90269_, this.f_90270_, this.f_90271_);
      }

      public String toString() {
         return "Segment{fullText='" + this.f_90269_ + "', linkTitle='" + this.f_90270_ + "', linkUrl='" + this.f_90271_ + "'}";
      }

      public String m_90278_() {
         return this.m_90284_() ? this.f_90270_ : this.f_90269_;
      }

      public boolean m_90284_() {
         return this.f_90270_ != null;
      }

      public String m_90285_() {
         if (!this.m_90284_()) {
            throw new IllegalStateException("Not a link: " + this);
         } else {
            return this.f_90271_;
         }
      }

      public static TextRenderingUtils.LineSegment m_90281_(String p_90282_, String p_90283_) {
         return new TextRenderingUtils.LineSegment((String)null, p_90282_, p_90283_);
      }

      @VisibleForTesting
      protected static TextRenderingUtils.LineSegment m_90279_(String p_90280_) {
         return new TextRenderingUtils.LineSegment(p_90280_);
      }
   }
}