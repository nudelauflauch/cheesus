package net.minecraft.advancements.critereon;

import com.google.gson.JsonElement;
import com.google.gson.JsonNull;
import com.google.gson.JsonObject;
import com.google.gson.JsonPrimitive;
import com.mojang.brigadier.StringReader;
import com.mojang.brigadier.exceptions.CommandSyntaxException;
import com.mojang.brigadier.exceptions.DynamicCommandExceptionType;
import com.mojang.brigadier.exceptions.SimpleCommandExceptionType;
import java.util.function.BiFunction;
import java.util.function.Function;
import java.util.function.Supplier;
import javax.annotation.Nullable;
import net.minecraft.network.chat.TranslatableComponent;
import net.minecraft.util.GsonHelper;

public abstract class MinMaxBounds<T extends Number> {
   public static final SimpleCommandExceptionType f_55297_ = new SimpleCommandExceptionType(new TranslatableComponent("argument.range.empty"));
   public static final SimpleCommandExceptionType f_55298_ = new SimpleCommandExceptionType(new TranslatableComponent("argument.range.swapped"));
   protected final T f_55299_;
   protected final T f_55300_;

   protected MinMaxBounds(@Nullable T p_55303_, @Nullable T p_55304_) {
      this.f_55299_ = p_55303_;
      this.f_55300_ = p_55304_;
   }

   @Nullable
   public T m_55305_() {
      return this.f_55299_;
   }

   @Nullable
   public T m_55326_() {
      return this.f_55300_;
   }

   public boolean m_55327_() {
      return this.f_55299_ == null && this.f_55300_ == null;
   }

   public JsonElement m_55328_() {
      if (this.m_55327_()) {
         return JsonNull.INSTANCE;
      } else if (this.f_55299_ != null && this.f_55299_.equals(this.f_55300_)) {
         return new JsonPrimitive(this.f_55299_);
      } else {
         JsonObject jsonobject = new JsonObject();
         if (this.f_55299_ != null) {
            jsonobject.addProperty("min", this.f_55299_);
         }

         if (this.f_55300_ != null) {
            jsonobject.addProperty("max", this.f_55300_);
         }

         return jsonobject;
      }
   }

   protected static <T extends Number, R extends MinMaxBounds<T>> R m_55306_(@Nullable JsonElement p_55307_, R p_55308_, BiFunction<JsonElement, String, T> p_55309_, MinMaxBounds.BoundsFactory<T, R> p_55310_) {
      if (p_55307_ != null && !p_55307_.isJsonNull()) {
         if (GsonHelper.m_13872_(p_55307_)) {
            T t2 = p_55309_.apply(p_55307_, "value");
            return p_55310_.m_55329_(t2, t2);
         } else {
            JsonObject jsonobject = GsonHelper.m_13918_(p_55307_, "value");
            T t = jsonobject.has("min") ? p_55309_.apply(jsonobject.get("min"), "min") : null;
            T t1 = jsonobject.has("max") ? p_55309_.apply(jsonobject.get("max"), "max") : null;
            return p_55310_.m_55329_(t, t1);
         }
      } else {
         return p_55308_;
      }
   }

   protected static <T extends Number, R extends MinMaxBounds<T>> R m_55313_(StringReader p_55314_, MinMaxBounds.BoundsFromReaderFactory<T, R> p_55315_, Function<String, T> p_55316_, Supplier<DynamicCommandExceptionType> p_55317_, Function<T, T> p_55318_) throws CommandSyntaxException {
      if (!p_55314_.canRead()) {
         throw f_55297_.createWithContext(p_55314_);
      } else {
         int i = p_55314_.getCursor();

         try {
            T t = m_55323_(m_55319_(p_55314_, p_55316_, p_55317_), p_55318_);
            T t1;
            if (p_55314_.canRead(2) && p_55314_.peek() == '.' && p_55314_.peek(1) == '.') {
               p_55314_.skip();
               p_55314_.skip();
               t1 = m_55323_(m_55319_(p_55314_, p_55316_, p_55317_), p_55318_);
               if (t == null && t1 == null) {
                  throw f_55297_.createWithContext(p_55314_);
               }
            } else {
               t1 = t;
            }

            if (t == null && t1 == null) {
               throw f_55297_.createWithContext(p_55314_);
            } else {
               return p_55315_.m_55332_(p_55314_, t, t1);
            }
         } catch (CommandSyntaxException commandsyntaxexception) {
            p_55314_.setCursor(i);
            throw new CommandSyntaxException(commandsyntaxexception.getType(), commandsyntaxexception.getRawMessage(), commandsyntaxexception.getInput(), i);
         }
      }
   }

   @Nullable
   private static <T extends Number> T m_55319_(StringReader p_55320_, Function<String, T> p_55321_, Supplier<DynamicCommandExceptionType> p_55322_) throws CommandSyntaxException {
      int i = p_55320_.getCursor();

      while(p_55320_.canRead() && m_55311_(p_55320_)) {
         p_55320_.skip();
      }

      String s = p_55320_.getString().substring(i, p_55320_.getCursor());
      if (s.isEmpty()) {
         return (T)null;
      } else {
         try {
            return p_55321_.apply(s);
         } catch (NumberFormatException numberformatexception) {
            throw p_55322_.get().createWithContext(p_55320_, s);
         }
      }
   }

   private static boolean m_55311_(StringReader p_55312_) {
      char c0 = p_55312_.peek();
      if ((c0 < '0' || c0 > '9') && c0 != '-') {
         if (c0 != '.') {
            return false;
         } else {
            return !p_55312_.canRead(2) || p_55312_.peek(1) != '.';
         }
      } else {
         return true;
      }
   }

   @Nullable
   private static <T> T m_55323_(@Nullable T p_55324_, Function<T, T> p_55325_) {
      return (T)(p_55324_ == null ? null : p_55325_.apply(p_55324_));
   }

   @FunctionalInterface
   protected interface BoundsFactory<T extends Number, R extends MinMaxBounds<T>> {
      R m_55329_(@Nullable T p_55330_, @Nullable T p_55331_);
   }

   @FunctionalInterface
   protected interface BoundsFromReaderFactory<T extends Number, R extends MinMaxBounds<T>> {
      R m_55332_(StringReader p_55333_, @Nullable T p_55334_, @Nullable T p_55335_) throws CommandSyntaxException;
   }

   public static class Doubles extends MinMaxBounds<Double> {
      public static final MinMaxBounds.Doubles f_154779_ = new MinMaxBounds.Doubles((Double)null, (Double)null);
      private final Double f_154780_;
      private final Double f_154781_;

      private static MinMaxBounds.Doubles m_154795_(StringReader p_154796_, @Nullable Double p_154797_, @Nullable Double p_154798_) throws CommandSyntaxException {
         if (p_154797_ != null && p_154798_ != null && p_154797_ > p_154798_) {
            throw f_55298_.createWithContext(p_154796_);
         } else {
            return new MinMaxBounds.Doubles(p_154797_, p_154798_);
         }
      }

      @Nullable
      private static Double m_154802_(@Nullable Double p_154803_) {
         return p_154803_ == null ? null : p_154803_ * p_154803_;
      }

      private Doubles(@Nullable Double p_154784_, @Nullable Double p_154785_) {
         super(p_154784_, p_154785_);
         this.f_154780_ = m_154802_(p_154784_);
         this.f_154781_ = m_154802_(p_154785_);
      }

      public static MinMaxBounds.Doubles m_154786_(double p_154787_) {
         return new MinMaxBounds.Doubles(p_154787_, p_154787_);
      }

      public static MinMaxBounds.Doubles m_154788_(double p_154789_, double p_154790_) {
         return new MinMaxBounds.Doubles(p_154789_, p_154790_);
      }

      public static MinMaxBounds.Doubles m_154804_(double p_154805_) {
         return new MinMaxBounds.Doubles(p_154805_, (Double)null);
      }

      public static MinMaxBounds.Doubles m_154808_(double p_154809_) {
         return new MinMaxBounds.Doubles((Double)null, p_154809_);
      }

      public boolean m_154810_(double p_154811_) {
         if (this.f_55299_ != null && this.f_55299_ > p_154811_) {
            return false;
         } else {
            return this.f_55300_ == null || !(this.f_55300_ < p_154811_);
         }
      }

      public boolean m_154812_(double p_154813_) {
         if (this.f_154780_ != null && this.f_154780_ > p_154813_) {
            return false;
         } else {
            return this.f_154781_ == null || !(this.f_154781_ < p_154813_);
         }
      }

      public static MinMaxBounds.Doubles m_154791_(@Nullable JsonElement p_154792_) {
         return m_55306_(p_154792_, f_154779_, GsonHelper::m_144769_, MinMaxBounds.Doubles::new);
      }

      public static MinMaxBounds.Doubles m_154793_(StringReader p_154794_) throws CommandSyntaxException {
         return m_154799_(p_154794_, (p_154807_) -> {
            return p_154807_;
         });
      }

      public static MinMaxBounds.Doubles m_154799_(StringReader p_154800_, Function<Double, Double> p_154801_) throws CommandSyntaxException {
         return m_55313_(p_154800_, MinMaxBounds.Doubles::m_154795_, Double::parseDouble, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidDouble, p_154801_);
      }
   }

   public static class Ints extends MinMaxBounds<Integer> {
      public static final MinMaxBounds.Ints f_55364_ = new MinMaxBounds.Ints((Integer)null, (Integer)null);
      private final Long f_55365_;
      private final Long f_55366_;

      private static MinMaxBounds.Ints m_55377_(StringReader p_55378_, @Nullable Integer p_55379_, @Nullable Integer p_55380_) throws CommandSyntaxException {
         if (p_55379_ != null && p_55380_ != null && p_55379_ > p_55380_) {
            throw f_55298_.createWithContext(p_55378_);
         } else {
            return new MinMaxBounds.Ints(p_55379_, p_55380_);
         }
      }

      @Nullable
      private static Long m_55384_(@Nullable Integer p_55385_) {
         return p_55385_ == null ? null : p_55385_.longValue() * p_55385_.longValue();
      }

      private Ints(@Nullable Integer p_55369_, @Nullable Integer p_55370_) {
         super(p_55369_, p_55370_);
         this.f_55365_ = m_55384_(p_55369_);
         this.f_55366_ = m_55384_(p_55370_);
      }

      public static MinMaxBounds.Ints m_55371_(int p_55372_) {
         return new MinMaxBounds.Ints(p_55372_, p_55372_);
      }

      public static MinMaxBounds.Ints m_154814_(int p_154815_, int p_154816_) {
         return new MinMaxBounds.Ints(p_154815_, p_154816_);
      }

      public static MinMaxBounds.Ints m_55386_(int p_55387_) {
         return new MinMaxBounds.Ints(p_55387_, (Integer)null);
      }

      public static MinMaxBounds.Ints m_154819_(int p_154820_) {
         return new MinMaxBounds.Ints((Integer)null, p_154820_);
      }

      public boolean m_55390_(int p_55391_) {
         if (this.f_55299_ != null && this.f_55299_ > p_55391_) {
            return false;
         } else {
            return this.f_55300_ == null || this.f_55300_ >= p_55391_;
         }
      }

      public boolean m_154817_(long p_154818_) {
         if (this.f_55365_ != null && this.f_55365_ > p_154818_) {
            return false;
         } else {
            return this.f_55366_ == null || this.f_55366_ >= p_154818_;
         }
      }

      public static MinMaxBounds.Ints m_55373_(@Nullable JsonElement p_55374_) {
         return m_55306_(p_55374_, f_55364_, GsonHelper::m_13897_, MinMaxBounds.Ints::new);
      }

      public static MinMaxBounds.Ints m_55375_(StringReader p_55376_) throws CommandSyntaxException {
         return m_55381_(p_55376_, (p_55389_) -> {
            return p_55389_;
         });
      }

      public static MinMaxBounds.Ints m_55381_(StringReader p_55382_, Function<Integer, Integer> p_55383_) throws CommandSyntaxException {
         return m_55313_(p_55382_, MinMaxBounds.Ints::m_55377_, Integer::parseInt, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidInt, p_55383_);
      }
   }
}