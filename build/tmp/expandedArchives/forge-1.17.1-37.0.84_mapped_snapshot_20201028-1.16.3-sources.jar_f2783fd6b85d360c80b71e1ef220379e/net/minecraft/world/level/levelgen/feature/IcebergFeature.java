...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
   ...
   public boolean m_142674_(FeaturePlaceContext<BlockStateConfiguration> p_159884_) {
      ...
      Random random = p_159884_.m_159776_();
      ...
      for(int l1 = -k1; l1 < k1; ++l1) {
         for(int i2 = -k1; i2 < k1; ++i2) {
            for(int j2 = 0; j2 < l; ++j2) {
               int k2 = flag1 ? this.m_66109_(j2, l, j1) : this.m_66094_(random, j2, l, j1);
               if (flag1 || l1 < k2) {
                  this.m_66058_(worldgenlevel, random, blockpos, l, l1, j2, i2, k2, k1, flag1, j, d0, flag, blockstate);
               }
            }
         }
      }

      this.m_66051_(worldgenlevel, blockpos, j1, l, flag1, i);

      for(int i3 = -k1; i3 < k1; ++i3) {
         for(int j3 = -k1; j3 < k1; ++j3) {
            for(int k3 = -1; k3 > -i1; --k3) {
               int l3 = flag1 ? Mth.m_14167_((float)k1 * (1.0F - (float)Math.pow((double)k3, 2.0D) / ((float)i1 * 8.0F))) : k1;
               int l2 = this.m_66113_(random, -k3, i1, j1);
               if (i3 < l2) {
                  this.m_66058_(worldgenlevel, random, blockpos, i1, i3, k3, j3, l2, l3, flag1, j, d0, flag, blockstate);
               }
            }
         }
      }
      ...
      if (flag2) {
         this.m_66099_(random, worldgenlevel, j1, l, blockpos, flag1, i, d0, j);
      }

      return true;
   }

   private void m_66099_(Random p_66100_, LevelAccessor p_66101_, int p_66102_, int p_66103_, BlockPos p_66104_, boolean p_66105_, int p_66106_, double p_66107_, int p_66108_) {
      ...
      for(int i1 = 0; i1 < p_66103_ - 3; ++i1) {
         int j1 = this.m_66094_(p_66100_, i1, p_66103_, p_66102_);
         this.m_66035_(j1, i1, p_66104_, p_66101_, false, d0, blockpos, p_66106_, p_66108_);
      }

      for(int k1 = -1; k1 > -p_66103_ + p_66100_.nextInt(5); --k1) {
         int l1 = this.m_66113_(p_66100_, -k1, p_66103_, p_66102_);
         this.m_66035_(l1, k1, p_66104_, p_66101_, true, d0, blockpos, p_66106_, p_66108_);
      }
   ...
   private void m_66035_(int p_66036_, int p_66037_, BlockPos p_66038_, LevelAccessor p_66039_, boolean p_66040_, double p_66041_, BlockPos p_66042_, int p_66043_, int p_66044_) {
      ...
      for(int k = -i; k < i; ++k) {
         for(int l = -i; l < i; ++l) {
            ...
            if (d0 < 0.0D) {
               BlockPos blockpos = p_66038_.m_142082_(k, p_66037_, l);
               BlockState blockstate = p_66039_.m_8055_(blockpos);
               if (m_159885_(blockstate) || blockstate.m_60713_(Blocks.f_50127_)) {
                  if (p_66040_) {
                     this.m_5974_(p_66039_, blockpos, Blocks.f_49990_.m_49966_());
                  } else {
                     this.m_5974_(p_66039_, blockpos, Blocks.f_50016_.m_49966_());
                     this.m_66048_(p_66039_, blockpos);
                  }
               }
            }
         }
   ...
   private void m_66058_(LevelAccessor p_66059_, Random p_66060_, BlockPos p_66061_, int p_66062_, int p_66063_, int p_66064_, int p_66065_, int p_66066_, int p_66067_, boolean p_66068_, int p_66069_, double p_66070_, boolean p_66071_, BlockState p_66072_) {
      double d0 = p_66068_ ? this.m_66022_(p_66063_, p_66065_, BlockPos.f_121853_, p_66067_, this.m_66018_(p_66064_, p_66062_, p_66069_), p_66070_) : this.m_66029_(p_66063_, p_66065_, BlockPos.f_121853_, p_66066_, p_66060_);
      if (d0 < 0.0D) {
         BlockPos blockpos = p_66061_.m_142082_(p_66063_, p_66064_, p_66065_);
         double d1 = p_66068_ ? -0.5D : (double)(-6 - p_66060_.nextInt(3));
         if (d0 > d1 && p_66060_.nextDouble() > 0.9D) {
            return;
         }

         this.m_66085_(blockpos, p_66059_, p_66060_, p_66062_ - p_66064_, p_66062_, p_66068_, p_66071_, p_66072_);
      }

   }

   private void m_66085_(BlockPos p_66086_, LevelAccessor p_66087_, Random p_66088_, int p_66089_, int p_66090_, boolean p_66091_, boolean p_66092_, BlockState p_66093_) {
      BlockState blockstate = p_66087_.m_8055_(p_66086_);
      if (blockstate.m_60767_() == Material.f_76296_ || blockstate.m_60713_(Blocks.f_50127_) || blockstate.m_60713_(Blocks.f_50126_) || blockstate.m_60713_(Blocks.f_49990_)) {
         boolean flag = !p_66091_ || p_66088_.nextDouble() > 0.05D;
         int i = p_66091_ ? 3 : 2;
         if (p_66092_ && !blockstate.m_60713_(Blocks.f_49990_) && (double)p_66089_ <= (double)p_66088_.nextInt(Math.max(1, p_66090_ / i)) + (double)p_66090_ * 0.6D && flag) {
            this.m_5974_(p_66087_, p_66086_, Blocks.f_50127_.m_49966_());
         } else {
            this.m_5974_(p_66087_, p_66086_, p_66093_);
         }
      }
   ...
   private double m_66029_(int p_66030_, int p_66031_, BlockPos p_66032_, int p_66033_, Random p_66034_) {
      float f = 10.0F * Mth.m_14036_(p_66034_.nextFloat(), 0.2F, 0.8F) / (float)p_66033_;
      return (double)f + Math.pow((double)(p_66030_ - p_66032_.m_123341_()), 2.0D) + Math.pow((double)(p_66031_ - p_66032_.m_123343_()), 2.0D) - Math.pow((double)p_66033_, 2.0D);
   }
   ...
   private int m_66094_(Random p_66095_, int p_66096_, int p_66097_, int p_66098_) {
   ...
   private int m_66113_(Random p_66114_, int p_66115_, int p_66116_, int p_66117_) {
      ...
      for(int j = -i; j <= i; ++j) {
         for(int k = -i; k <= i; ++k) {
            for(int l = 0; l <= p_66055_; ++l) {
               BlockPos blockpos = p_66053_.m_142082_(j, l, k);
               ...
               if (m_159885_(blockstate) || blockstate.m_60713_(Blocks.f_50125_)) {
                  if (this.m_66045_(p_66052_, blockpos)) {
                     ...
                     BlockState[] ablockstate = new BlockState[]{p_66052_.m_8055_(blockpos.m_142125_()), p_66052_.m_8055_(blockpos.m_142126_()), p_66052_.m_8055_(blockpos.m_142127_()), p_66052_.m_8055_(blockpos.m_142128_())};
                     int i1 = 0;

                     for(BlockState blockstate1 : ablockstate) {
                        if (!m_159885_(blockstate1)) {
                           ++i1;
                        }
                     }
