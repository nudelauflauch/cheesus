...
public enum B3DClip implements IClip
{
    ...
    protected static class NodeClip implements IJointClip
    {
        ...
        public Transformation apply(float time)
        {
            ...
            {
                return ret.m_121096_(new Transformation(node.getPos(), node.getRot(), node.getScale(), null));
            }
            ...
            if(keyStart == null)
            {
                ...
                {
                    ret = ret.m_121096_(endTr);
                }
            }
            else if(progress < 1e-5 || keyEnd == null)
            {
                ret = ret.m_121096_(startTr);
            }
            else
            {
                ret = ret.m_121096_(TransformationHelper.slerp(startTr, endTr, progress));
            }
            return ret;
        }
    }
