...
public class BlockInfo
{
    ...
    public int getColorMultiplier(int tint)
    {
        ...
        cachedTint = tint;
        cachedMultiplier = colors.m_92577_(state, world, blockPos, tint);
    ...
    public void updateLightMatrix()
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
            {
                for(int z = 0; z <= 2; z++)
                {
                    ...
                    t[x][y][z] = state.m_60739_(world, pos) < 15;
                    int brightness = LevelRenderer.m_109541_(world, pos);
                    s[x][y][z] = LightTexture.m_109894_(brightness);
                    b[x][y][z] = LightTexture.m_109883_(brightness);
                    ao[x][y][z] = state.m_60792_(world, pos);
                }
            }
        }
        for(Direction side : SIDES)
        {
            ...
            if(state.m_60739_(world, pos) == 15 || Shapes.m_83145_(thisStateShape.m_60655_(world, blockPos, side), otherStateShape.m_60655_(world, pos, side.m_122424_())))
            {
    ...
    {
        full = Block.m_49916_(state.m_60812_(world, blockPos));
        packed[0] = LevelRenderer.m_109541_(world, blockPos);

        for (Direction side : SIDES)
        {
            int i = side.ordinal() + 1;
            packed[i] = LevelRenderer.m_109541_(world, blockPos.m_142300_(side));
        }
    }
