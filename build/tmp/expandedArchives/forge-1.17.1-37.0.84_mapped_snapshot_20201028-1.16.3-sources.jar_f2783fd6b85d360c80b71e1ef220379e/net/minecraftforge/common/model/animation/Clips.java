...
public final class Clips
{
    ...
    private static IJointClip blendClips(final IJoint joint, final IJointClip fromClip, final IJointClip toClip, final ITimeValue input, final ITimeValue progress)
    {
        return new IJointClip()
        {
            ...
            {
                float clipTime = input.apply(time);
                return TransformationHelper.slerp(fromClip.apply(clipTime), toClip.apply(clipTime), Mth.m_14036_(progress.apply(time), 0, 1));
            }
        };
    }
    ...
    public static Pair<ModelState, Iterable<Event>> apply(final IClip clip, final float lastPollTime, final float time)
    {
        return Pair.of(new ModelState()
        {
            ...
            public Transformation getPartTransformation(Object part)
            {
                ...
                Transformation jointTransform = clip.apply(joint).apply(time).m_121096_(joint.getInvBindPose());
                Optional<? extends IJoint> parent = joint.getParent();
                while(parent.isPresent())
                {
                    Transformation parentTransform = clip.apply(parent.get()).apply(time);
                    jointTransform = parentTransform.m_121096_(jointTransform);
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type)
        {
            ...
            return (TypeAdapter<T>)new TypeAdapter<IClip>()
            {
                ...
                public void write(JsonWriter out, IClip clip) throws IOException
                {
                    ...
                    {
                        out.value("#" + ((StringRepresentable)clip).m_7912_());
                        return;
                    }
