...
public class ForgeChunkManager
{
    ...
    public static void writeForgeForcedChunks(CompoundTag nbt, TicketTracker<BlockPos> blockForcedChunks, TicketTracker<UUID> entityForcedChunks)
    {
        if (!blockForcedChunks.isEmpty() || !entityForcedChunks.isEmpty())
        {
            ...
            writeForcedChunkOwners(forcedEntries, blockForcedChunks, "Blocks", Constants.NBT.TAG_COMPOUND, (pos, forcedBlocks) -> forcedBlocks.add(NbtUtils.m_129224_(pos)));
            writeForcedChunkOwners(forcedEntries, entityForcedChunks, "Entities", Constants.NBT.TAG_INT_ARRAY, (uuid, forcedEntities) -> forcedEntities.add(NbtUtils.m_129226_(uuid)));
    ...
    {
        ListTag forcedChunks = nbt.m_128437_("ForgeForced", Constants.NBT.TAG_COMPOUND);
        for (int i = 0; i < forcedChunks.size(); i++)
        {
            ...
            if (ModList.get().isLoaded(modId))
            {
                ListTag modForced = forcedEntry.m_128437_("ModForced", Constants.NBT.TAG_COMPOUND);
    ...
    {
        ListTag forcedBlocks = modEntry.m_128437_(key, Constants.NBT.TAG_COMPOUND);
    ...
    {
        ListTag forcedEntities = modEntry.m_128437_(key, Constants.NBT.TAG_INT_ARRAY);
