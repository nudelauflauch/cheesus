...
public class GameData
{
    ...
    {
        return new RegistryBuilder<T>().setName(key.m_135782_()).setType(type).setMaxID(MAX_VARINT).addCallback(new NamespacedWrapper.Factory<T>());
    }
    ...
    public static void setCustomTagTypesFromRegistries()
    {
        ...
        for (Map.Entry<ResourceLocation, ForgeRegistry<? extends IForgeRegistryEntry<?>>> entry : RegistryManager.ACTIVE.registries.entrySet())
        {
            ...
            if (entry.getValue().getTagFolder() != null && StaticTags.get(registryName) == null)
            {
                ...
                StaticTags.m_144351_(ResourceKey.m_135788_(registryName), "tags/" + entry.getValue().getTagFolder());
            }
        }
            ...
            if (oldBlock != null)
            {
                ...
                if (block.getRegistryName().m_135827_().equals("minecraft") && !oldContainer.m_61092_().equals(newContainer.m_61092_()))
                {
                    String oldSequence = oldContainer.m_61092_().stream()
                            .map(s -> String.format("%s={%s}", s.m_61708_(),
